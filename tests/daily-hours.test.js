...
test('seuls les jours choisis apparaissent, y compris après déplacement de la demi-journée', () => {
  ...
  expect(dates.filter((date) => entryOccursOn(entry, date))).toEqual([
    '2026-09-01',
    '2026-09-03',
    '2026-09-04',
  ])
...
test('les anciennes plages restent compatibles ; aucune répartition inventée', () => {
  expect(
    entryOccursOn({ kind: 'equipe', starts_on: '2026-09-01', ends_on: '2026-09-30' }, '2026-09-06'),
  ).toBe(true)
...
test('refuse les dates invalides et doublons', () => {
  ...
  expect(() =>
    parseDailyHours(JSON.stringify([{ ...row, date: '2026-02-30' }]), '2026-02-01', '2026-02-31'),
  ).toThrow()
})
