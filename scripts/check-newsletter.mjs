...
try {
  for (const width of [390, 1440]) {
    ...
    for (const [path, formId, statusId] of [
      ...
      await expect(form.locator('[name="profil"]')).toHaveCount(4)
      await expect(form.locator('label').filter({ hasText: 'Je souhaite devenir membre/partenaire' })).toHaveCount(1)
      await expect(form.locator('label').filter({ hasText: 'Je m’intéresse à vos enjeux' })).toHaveCount(1)
      ...
      for (const state of ['deja-inscrit', 'technique', 'network', 'html']) {
        ...
        await expect(form.locator('[value="enjeux"]')).toBeChecked()
