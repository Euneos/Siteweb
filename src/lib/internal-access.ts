...
export async function requireInternalAccess(
  ...
  if (!domain || !/^[a-z0-9-]+\.cloudflareaccess\.com$/.test(domain) || !audience) {
    return new Response('Cet espace interne n’est pas encore disponible. Contactez votre administrateur.', { status: 503, headers: privateHeaders })
  }
  const token = request.headers.get('Cf-Access-Jwt-Assertion')
  if (!token) return new Response('Connectez-vous avec votre compte autorisé pour consulter cet espace.', { status: 403, headers: privateHeaders })
  try {
    ...
    if (typeof payload.email !== 'string' || !payload.email.trim()) throw new Error('Missing identity')
    return null
  } catch {
    return new Response('Votre accès est absent ou a expiré. Reconnectez-vous avec votre compte autorisé.', { status: 403, headers: privateHeaders })
  }
}
