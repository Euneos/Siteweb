...
export function initDailyHours(form: HTMLFormElement) {
  ...
  let enabled = false
  const dates = () => {
    const start = field('starts_on').value, end = field('ends_on').value
  ...
  const rows = () => parseDailyHours(stored.value, field('starts_on').value, field('ends_on').value) ?? []
  ...
  function action(text: string, run: () => void) {
    ...
    button.addEventListener('click', () => {
      try { run() } catch (error) { feedback.textContent = (error as Error).message }
    })
    return button
  }
  function render(team: boolean) {
    ...
    if (!team) return
    if (!stored.value) {
      feedback.textContent = 'Cette fiche couvre toute la période. Passez à la saisie par jour pour choisir uniquement les dates travaillées.'
      controls.append(action('Saisir les heures par jour', () => {
        dates()
        if (total.value !== '' && !window.confirm(`Cette fiche contient un total de ${total.value} h sans répartition. En passant à la saisie par jour, ce total sera remplacé par les heures réalisées que vous renseignerez. Continuer ?`)) return
        stored.value = '[]'
        render(true)
        notify()
      }))
    ...
    instructions.textContent = 'Pour la demi-journée variable, utilisez « Prévoir 3 h 30 » sur le mercredi ou vendredi choisi chaque semaine. Effacez une ancienne prévision pour déplacer cette demi-journée. Les valeurs déjà saisies ne sont pas remplacées par le préremplissage.'
    controls.append(instructions)
    for (const date of days) {
      const row = existing.find((row) => row.date === date)
      const group = document.createElement('div')
      group.className = 'iw-formgrid'
      const day = new Date(`${date}T12:00:00Z`)
      const heading = document.createElement('p')
      heading.className = 'iw-field--wide'
      heading.textContent = new Intl.DateTimeFormat('fr-FR', { weekday: 'long', day: 'numeric', month: 'long', timeZone: 'UTC' }).format(day)
      ...
      for (const key of ['planned', 'actual'] as const) {
        ...
        input.type = 'number'
        input.min = '0'; input.max = '24'; input.step = 'any'
        ...
        input.addEventListener('input', () => {
          ...
          if (!entry) { entry = { date, planned: null, actual: null }; current.push(entry) }
          entry[key] = input.value === '' ? null : Number(input.value)
          stored.value = JSON.stringify(current.filter((row) => row.planned !== null || row.actual !== null))
          updateTotal()
          notify()
        })
        label.append(input)
        ...
        if (key === 'planned' && [3, 5].includes(day.getUTCDay())) {
          group.append(action('Prévoir 3 h 30', () => {
            input.value = '3.5'
  ...
  for (const name of ['starts_on', 'ends_on']) field(name).addEventListener('change', () => render(enabled))
  function validate() {
    if (!enabled || !stored.value) return true
    try { dates(); rows(); updateTotal(); return true }
    catch (error) { feedback.textContent = (error as Error).message; return false }
  }
